...
class BlockHeaderClient( BitcoinBasicClient ):
    ...
    def handle_headers( self, message_header, block_headers_message ):
        ...
        for i in range(header_start, len(block_headers)):
...
class SPVClient(object):
    ...
    def block_header_index( cls, path, block_header ):
        ...
        for blk in range(0, len(chain_raw) / (BLOCK_HEADER_SIZE)):
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("Usage: %s bitcoind_server headers_path blockchain_height" % sys.argv[0], file=sys.stderr)
        sys.exit(0)
