...
def parse_multisig_redeemscript( redeem_script_hex ):
    ...
    try:
        ...
        if os.environ.get("BLOCKSTACK_TEST") == "1":
            traceback.print_exc()
            print("Invalid redeem script %s" % redeem_script_hex, file=sys.stderr)
    ...
    try:
        ...
        if os.environ.get("BLOCKSTACK_TEST") == "1":
            traceback.print_exc()
            print("Invalid redeem script %s (parses to %s)" % (redeem_script_hex, script_parts), file=sys.stderr)
