...
def get_logger(name=None):
    ...
    if len(log.handlers) > 0:
        for i in range(0, len(log.handlers)):
            log.handlers.pop(0)
...
class BitcoindConnection( http.client.HTTPSConnection ):
   """
...
def create_bitcoind_connection( rpc_username, rpc_password, server, port, use_https, timeout ):
    ...
    if use_https:
        ...
        if do_wrap_socket:
           ...
           connection = http.client.HTTPSConnection( server, int(port), context=ssl_ctx, timeout=timeout )
           ret = AuthServiceProxy(authproxy_config_uri, connection=connection)
...
def connect_bitcoind_impl( bitcoind_opts ):
    ...
    if 'bitcoind_port' in list(bitcoind_opts.keys()) and bitcoind_opts['bitcoind_port'] is None:
        log.error("No port given")
        raise ValueError("No RPC port given (bitcoind_port)")

    if 'bitcoind_timeout' in list(bitcoind_opts.keys()) and bitcoind_opts['bitcoind_timeout'] is None:
        # default
