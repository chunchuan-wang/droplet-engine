...
class BlockchainDownloader( BitcoinBasicClient ):
    ...
    def __init__(self, bitcoind_opts, spv_headers_path, first_block_height, last_block_height, p2p_port=None, sock=None, tx_filter=None ):
        ...
        for i in range(first_block_height, last_block_height + 1):
    ...
    def get_block_info(self):
        ...
        ret = []
        for (block_hash, block_data) in list(self.block_info.items()):
    ...
    def fetch_sender_txs(self):
        ...
        # fetch remaining sender transactions
        if len(list(self.sender_info.keys())) > 0:

            sender_txids = list(self.sender_info.keys())[:]
            sender_txid_batches = []
            batch_size = 20

            for i in range(0, len(sender_txids), batch_size ):
                sender_txid_batches.append( sender_txids[i:i+batch_size] )

            for i in range(0, len(sender_txid_batches)):
                ...
                for sender_txid, sender_tx in list(sender_txs.items()):

                    assert sender_txid in list(self.sender_info.keys()), "Unsolicited sender tx %s" % sender_txid

                    # match sender outputs to the nulldata tx's inputs
                    for nulldata_input_vout_index in list(self.sender_info[sender_txid].keys()):
    ...
    def block_data_sanity_checks(self):
        ...
        assert self.num_txs_received == len(list(self.sender_info.keys())), "Num TXs received: %s; num TXs requested: %s" % (self.num_txs_received, len(list(self.sender_info.keys())))

        for (block_hash, block_info) in list(self.block_info.items()):
            for tx in block_info['txns']:
                ...
                for i in range(0, len(tx['vin'])):
                    ...
                    assert sinfo['txid'] in self.sender_info, "Surreptitious sender tx %s" % sinfo['txid']
    ...
    def handle_inv(self, message_header, inv_packet ):
        ...
        for inv_info in inv_packet.inventory:
            ...
            if inv_info.inv_type == INVENTORY_TYPE["MSG_BLOCK"]:
                ...
                if inv_hash in list(self.block_info.keys()) and not self.block_info[inv_hash]['handled']:
    ...
    def add_sender_info( self, sender_txhash, nulldata_vin_outpoint, sender_out_data ):
        ...
        """
        assert sender_txhash in list(self.sender_info.keys()), "Missing sender info for %s" % sender_txhash
    ...
    def parse_tx( self, txn, block_header, block_hash, txindex ):
        ...
        for i in range(0, len(txn.tx_out)):
            outp = txn.tx_out[i]
    ...
    def handle_block( self, message_header, block ):
        ...
        * ask for each transaction's sender transaction
        ...
        if block_hash not in list(self.block_info.keys()):
        ...
        for txindex in range(0, len(block.txns)):
            ...
            for outp in txdata['vout']:
                if outp['scriptPubKey']['type'] == 'nulldata':
                    ...
                    if type(nulldata_payload) not in [str, str]:
        ...
        for txn in self.block_info[block_hash]['txns']:
            for i in range(0, len(txn['vin'])):
                ...
                if sender_txid not in self.sender_info:
    ...
    def fetch_txs_rpc( self, bitcoind_opts, txids ):
        ...
        proto = "http"
        if 'bitcoind_use_https' in bitcoind_opts and bitcoind_opts['bitcoind_use_https']:
            proto = "https"
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("Usage: %s bitcoind_server headers_path blockchain_height block_start_height" % sys.argv[0], file=sys.stderr)
        sys.exit(0)
    ...
    # test get blocks
    for interval in range(start_height, height, 20):
        ...
        for block_hash, block_info in list(blocks.items()):
