...
from . import config
from . import blockchain.transactions as transactions
from . import blockchain.session as session
...
class StateEngine( object ):
    ...
    * "Saving" means writing the new state to persistent storage.
    ...
    def process_block( self, block_id, ops, backup=False, expected_snapshots=None ):
        ...
        # sanity check 
        if block_id in expected_snapshots and expected_snapshots[block_id] != consensus_hash:
            log.error("FATAL: consensus hash mismatch at height %s: %s != %s" % (block_id, expected_snapshots[block_id], consensus_hash))
            traceback.print_stack()
            os.abort()

        for op in list(new_ops.keys()):

            sanitized_ops[op] = []
            for i in range(0, len(new_ops[op])):
    ...
    def get_block_from_consensus( self, consensus_hash ):
        ...
        for (block_id, ch) in self.consensus_hashes.items():
    ...
    def get_valid_consensus_hashes( self, block_id ):
        ...
        for block_number in range(first_block_to_check, block_id+1):
            
            block_number_key = str(block_number)
            
            if block_number_key not in list(self.consensus_hashes.keys()):
                continue
